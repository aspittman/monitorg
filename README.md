...
**Daily/monthly/total bot returns:** currently N/A for all eight bots. No inspected source provides a complete, trusted bot-level valuation/cash-flow series with the required boundaries. The guarded helper uses `100 × (E_end / E_start − 1)` only for a positive verified bot starting equity and no intervening external flows. For daily return `E_start` would be the prior-day boundary valuation; for MTD it would be the month boundary; for total it would be a verified inception valuation. Those inputs are not synthesized. With intervening flows, the helper rejects the calculation; flow-adjusted/time-weighted returns are not yet implemented. Current risk budgets, virtual capital settings, account equity histories, and open-position percentage gains are not substituted.
