...
## Stop-loss chart lines (September 25, 2026)

The shared Trade Inspector chart now draws amber stop levels from the first recorded threshold. A single threshold draws a horizontal line; later updates form steps. Separate `stop_id` values remain separate lines, and `active: false` ends a line. Lines end at closure or the replay cutoff. Dashed horizontal segments mean **last recorded level carried forward**, not proof that no unrecorded changes occurred. Underlying stops and option-premium stops remain on their own price basis. Entry/exit markers and actual market history are unchanged.

`GET /api/inspector` additionally returns `stop_history` and `stop_warning`. Stop evidence is independent of timeline pagination, bounded to 10,000 snapshots with a visible truncation warning. No new route or database migration is required. Journal payloads retain optional legacy stop evidence; the main dashboard/recent-activity responses omit these arrays to keep polling light.

Sources and limits:

- All eight bots support normalized `risk.initial_stop` / `risk.current_stop` telemetry with explicit `price_basis`. Multiple mechanisms may use `risk.stops`, with `stop_id`, `label`, `price_basis`, `current_stop` (or `initial_stop`), and optional `active: false`. An update must retain the same ID. Emit `STOP_UPDATED` at the actual creation/change/removal time with the existing trade or order ID. Do not populate thresholds by reevaluating old trades with today's settings.
- CCExchange: reads recorded BUY/SELL submission stops from `logs/events.jsonl`, joined by exact order ID, symbol, and side to the submission timestamp in `paper_data/orders.csv`. Untimestamped `POSITION_MANAGED` rows cannot establish historical changes and are excluded. Initial and exit stop observations do not provide the intervening trajectory.
- ETFEnhancer: the current structural stop is read only for an open current position whose exact entry order ID matches `position_state_<mode>.json`. It is explicitly labeled **Current reference**; its creation time is unknown and the horizontal reference is not a historical trajectory.
- momentum_master: open owned positions can display the local current trailing stop from `state/bot_state.json`, labeled as a current reference with creation time and broker acceptance unverified. Closed trades do not receive current stop values.
- options_direct / options_inverted: recorded `RISK_SNAPSHOT` and `OPTION_TRAIL_SNAPSHOT` rows are attached through the ledger's opening-fill lifecycle for the same strategy and contract. Partial exits retain the cycle; closure ends it; adoption/missing-position records invalidate linkage. Repeated identical levels are omitted. Underlying and premium thresholds are kept separate. No configuration-derived stop prices are synthesized.
- ETFEnhancerLT, options_covered, options_secured, and any historical trade lacking linked snapshots show stop data unavailable until the bot exports it. The generic renderer already supports those bots; absence of a recorded stop is not proof that no stop existed.

No trading bot code, strategy, broker order, or stop is modified. Monitoring remains read-only. Changed files: `adapters/options_events.py`, `services/explain_service.py`, `services/monitor.py`, `static/explain.js`, `tests/explain_ui.cjs`, and this document. New files: `services/trade_stops.py`, `tests/test_trade_stops.py`. Verify with `python3 -m unittest discover -q` and `node --harmony-optional-chaining --harmony-nullish tests/explain_ui.cjs`. Rollback: revert only this stop-overlay patch and restart MonitorG; existing databases, bots, and trades require no rollback.
