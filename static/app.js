...
function render() {
  ...
  $('bots').innerHTML=dashboard.bots.map(b=>`<tr class="${b.bot_id===selected?'selected':''}"><td><button class="bot-button" data-bot="${esc(b.bot_id)}">${esc(b.bot_id)}</button></td><td><span class="badge ${esc(b.status)}" title="${esc(b.reason)}">${esc(b.status)}</span></td><td title="${esc(b.position_confidence)}">${number(b.open_positions)}</td><td>${number(b.trades_today)}</td><td>${number(b.trades_month)}</td><td>${percent(b.daily_return)}</td><td>${percent(b.monthly_return)}</td><td>${percent(b.total_return)}</td><td><span class="badge ${b.monitor_status==='OK'&&b.warnings.length?'REVIEW':esc(b.monitor_status)}">${b.monitor_status==='OK'&&b.warnings.length?'REVIEW':esc(b.monitor_status)}</span></td></tr>`).join('');
...
function drawChart(error) {
  ...
  for(let i=0;i<4;i++){const value=min+(max-min)*i/3,py=y(value);ctx.strokeStyle='#243247';ctx.beginPath();ctx.moveTo(left,py);ctx.lineTo(left+width,py);ctx.stroke();ctx.fillStyle='#90a1b8';ctx.fillText(value.toFixed(1),4,py);}
